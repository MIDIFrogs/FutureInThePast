...
public abstract class SerializableDictionary : ICollection
{
    public abstract int Count { get; }
    public abstract bool IsSynchronized { get; }
    public abstract object SyncRoot { get; }

    public void CopyTo(Array array, int index) => CopyToInternal(array, index);
...
public class SerializableDictionary<TKey, TValue> : SerializableDictionary, IDictionary<TKey, TValue>, ISerializationCallbackReceiver
{
    ...
    public override object SyncRoot => (list as ICollection).SyncRoot;
    public override bool IsSynchronized => (list as ICollection).IsSynchronized;
    ...
    #region Non-Generic SerializableDictionary
    protected override void CopyToInternal(Array array, int index)
    {
        if (array is KeyValuePair<TKey, TValue>[] arr)
            CopyTo(arr, index);
        else throw new ArrayTypeMismatchException("Cannot copy to the target array");
    }

    protected override IEnumerator GetEnumeratorInternal() => GetEnumerator();
    #endregion
}